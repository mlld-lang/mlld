...
export interface VarsCommandOptions {
  _: string[]; // Subcommand and arguments
  cwd?: string;
}

export async function varsCommand(options: VarsCommandOptions): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'allow':
      if (args.length === 0) {
        ...
        console.error('Usage: mlld vars allow <variable_name> [variable_name...]');
        process.exit(1);
      }
    ...
    case 'remove':
      if (args.length === 0) {
        console.error(chalk.red('Error: Variable name required'));
        console.error('Usage: mlld vars remove <variable_name> [variable_name...]');
        process.exit(1);
      }
...
async function listEnvVars(projectConfig: ProjectConfig, projectRoot: string): Promise<void> {
  ...
  if (allowedVars.length === 0) {
    ...
    console.log(chalk.gray(`\nTo allow environment variables, use: mlld vars allow <variable_name>`));
    return;
  }
