export const pattern = {
  ...
  test(error, mx) {
    ...
    if (!error.message || !error.message.includes('Shell redirection')) {
      return false;
    }
    
    // Check if we're in an exe context
    if (!mx.line.match(/\/?exe/)) {
      return false;
    }
    ...
    if (mx.lines && mx.lineNumber < mx.lines.length) {
      ...
      return nextFewLines.some(line => line && line.trim().match(/\/?when/));
    }
  ...
  enhance(error, mx) {
    ...
    const hasWhenBlock = nextLines.some(line => line && line.match(/\/?when/));
