...
async function runLoop(
  ...
  const resolvedLimit = await resolveLoopLimit(options.limit, env, options.sourceLocation);
  ...
  if (rateMs !== null && rateMs < 0) {
    throw new MlldDirectiveError(
      ...
      'loop',
      { location: options.sourceLocation, context: { rateMs } }
    );
  }
  ...
  for (let iteration = 1; ; iteration++) {
    ...
    await setLoopInputVariable(iterEnv, state);
    ...
    const iterationResult = await iterEnv.withExecutionContext('loop', loopCtx, async (): Promise<LoopIterationResult> => {
      ...
      return { status: 'continue', value: state };
    });
