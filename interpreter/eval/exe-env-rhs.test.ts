...
describe('exe box RHS', () => {
  it('applies box tools scope at call time from exe params', async () => {
  ...
  it('enforces box tools restrictions inside exe box RHS', async () => {
    const fileSystem = new MemoryFileSystem();
    const source = [
      '/exe @read() = `read-ok`',
      '/var tools @readOnlyTools = {',
  ...
  it('supports box RHS executables when invoked through run', async () => {
    const fileSystem = new MemoryFileSystem();
    const source = [
      '/exe @read() = `read-ok`',
      '/var tools @readOnlyTools = {',
