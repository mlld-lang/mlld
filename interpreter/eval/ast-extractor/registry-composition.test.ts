...
describe('ast extractor registry composition', () => {
  it('keeps extension routing and default fallback stable across extractAst and extractNames', async () => {
    const pythonSource = [
      'class Service:',
      '    def build(self):',
      '        return helper()',
    ...
    const pythonUsage = await extractAst(pythonSource, 'service.pyi', [
    ...
    const pythonNames = await extractNames(pythonSource, 'service.pyi');
    ...
    const cppNames = await extractNames(cppSource, 'worker.hxx', 'fn');
    ...
    const unknownExtensionResults = await extractAst(unknownExtensionSource, 'fallback.unknownext', [
