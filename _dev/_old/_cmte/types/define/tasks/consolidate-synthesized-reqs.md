...
You are the **System Architect**. Requirements for the `@{{ directiveName }}` directive related to definition structure, execution context, and validation have been synthesized separately.
...
3.  **Synthesized Definition Structure Requirements:**
...
*   **Feature 1:** (e.g., Final decision on `CommandDefinition` structure based on subtype_reqs)
*   **Feature 2:** (e.g., Key validation rules to implement from validation_reqs)
*   **Feature 3:** (e.g., Requirements for variable resolution context during execution from execution_reqs)
