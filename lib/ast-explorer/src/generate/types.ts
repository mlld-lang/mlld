...
export function generateTypeInterface(node: DirectiveNode): string {
  ...
  const name = (typeof node.raw?.identifier === 'string')
    ? node.raw.identifier
    : kind + '-' + subtype;
  ...
  const typeName = `${pascalName}DirectiveNode`;
...
function generateMockTextAssignmentType(): string {
  return `import { DirectiveNode, TypedDirectiveNode } from '@grammar/types/base';
 ...
 */
export function isTextAssignmentDirectiveNode(node: DirectiveNode): node is TextAssignmentDirectiveNode {
