...
<guidance>
Create the sign-verify atom at docs/src/atoms/security/sign-verify.md. Document the sign and verify primitives: (1) sign directive syntax with signer and algorithm, (2) verify directive for checking signatures, (3) how signatures are stored as metadata, (4) what happens on verification failure, (5) the verification flow for audit patterns. Reference signing-overview.md for the 'why'. Use the /mlld skill for language knowledge and validate all examples with 'mlld validate'.
</guidance>
...
IMPORTANT: Write your JSON response to /Users/adam/dev/mlld/j2bd/security/runs/2026-02-01-0/worker-m-e4d6-2.json using the Write tool. Write ONLY valid JSON.