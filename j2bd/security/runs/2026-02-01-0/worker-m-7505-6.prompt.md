...
<guidance>
Create docs/src/atoms/security/labels-influenced.md. Key points: (1) What 'influenced' means - LLM output from context containing untrusted data, (2) How it's auto-applied via policy rule 'untrusted-llms-get-influenced' in defaults.rules, (3) Label propagation through subsequent operations, (4) Why this matters for prompt injection defense. Reference existing atoms: labels-overview, labels-source-auto. Include working mlld example showing exe llm processing untrusted data and output receiving influenced label. Validate with mlld validate.
</guidance>
...
You are executing a task RIGHT NOW. This is not a session setup.

1. **Explore first** - Use Read, Glob, Grep tools to understand the codebase and existing docs
2. **Do the work** - Write or fix the documentation atom
3. **Validate** - Run `mlld validate` on any mlld code blocks
4. **Return results** - Write JSON with your completed work

## Response Format

When your work is complete, return JSON:
...
```json
{
  ...
  "work_done": {
    ...
    "files": {
      "docs/src/atoms/security/feature-name.md": "Full content of the file"
    },
    "commit_message": "Add feature-name documentation"
  },
  ...
  "standup": {
    ...
    "blockers": "What's blocking further progress (if any)",
    "next": "What should happen next"
  }
}
```

If status is "blocked" or "needs_human", explain why in friction_points.
If status is "partial", describe what remains in standup.next.


IMPORTANT: Write your JSON response to /Users/adam/dev/mlld/j2bd/security/runs/2026-02-01-0/worker-m-7505-6.json using the Write tool. Write ONLY valid JSON.