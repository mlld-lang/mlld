...
- Tool catalog entries now accept `description`, `instructions`, and `authorizable`. Catalog labels are applied to the surfaced tool call, and catalog `authorizable` defaults merge into `@policy.build(...)` / `@policy.validate(...)`.
...
- `policy.authorizations.authorizable` on base policy declares which `role:*` executables may authorize which tools. Authorization identity now comes from immutable exe role labels, not display overrides.
