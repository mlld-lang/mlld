...
### Fixed
- Shelf slot refs are now first-class runtime values instead of `StructuredValue` wrappers with an internal marker. They now survive wrapper executables, imported wrappers, `let` rebinding, `??`, `when`, and ordinary exec parameter transport, so calls like `@shelf.clear(@activeExecutionLogSlot)` no longer lose slot identity on the way to the shelf builtins.
- Shelf-only object-config boxes no longer create an implicit workspace MCP bridge. They still get the normal box workspace, but shelf-only wrappers around `exe llm` now preserve the non-bridge LLM tool path instead of manufacturing spurious `@mx.box` / `@mx.llm.inBox` state.
