...
- `store` declarations unify heterogeneous backends (APIs, files, CLIs, MCP tools) into a queryable data access pattern: `store @contacts = { find: @searchContacts, get: @getContact }`. Multi-entity stores use nested keys: `store @crm = { deals: { find: ..., get: ... }, contacts: { find: ... } }`. Store executables coerce results through their backing records.
