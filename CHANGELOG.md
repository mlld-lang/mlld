...
- Box shelf scope now supports aliased slot refs on both `read` and `write`. Aliased entries surface at `@fyi.shelf.<alias>` and remain slot-ref-capable, so agents can read projected contents directly and pass the same alias into `@shelve(...)`, `@shelve.read(...)`, `@shelve.clear(...)`, and `@shelve.remove(...)`.
