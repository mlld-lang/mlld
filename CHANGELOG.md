...
- **Before `op:exe` guard transform boundaries**: Combined before guards for data labels and `op:exe` now unwrap replacement values before reinsertion, so exec arguments receive resolved values instead of serialized internal Variable structures.
