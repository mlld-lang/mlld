...
- `@policy.build(...)` now auto-allows omitted bucketed tools whose input records declare no facts or only `optional_benign` fact fields, and the compile report records those decisions in `report.autoAllowedTools`.
...
- Record-backed single-parameter wrapper tools now infer whole-object dispatch from `inputs: @record`; `direct: true` is no longer required there, and `direct: false` remains an opt-out.
- Tool collections now expose compiled reflection through `.mx`: collections add `.mx.tools`, and entries expose `.mx.inputSchema`, `.mx.factArgs`, `.mx.dataArgs`, `.mx.optionalArgs`, `.mx.allowlist`, `.mx.blocklist`, `.mx.canAuthorize`, and related fields.
...
### Added
- CLI wrappers and subprocess SDK clients now support configurable Node heap limits with `--mlld-heap`, `MLLD_HEAP`, and `runtime.heap` in config. `--heap-snapshot-near-limit` passes through to V8 heap snapshot diagnostics.
- Runtime tracing now supports `--trace-memory`, adding `memory.*` trace events with RSS/heap/external/ArrayBuffer samples and deltas. `--trace-memory` implies `--trace effects` when no trace level is set, and all maintained SDKs plus live stdio requests accept memory trace options.
...
- Record fact fields now support `kind` tags and `accepts` overrides. `@policy.build(...)` derives control-arg fact requirements from matching kinds, while untagged fields keep the strict field-name fallback.
...
- Role-qualified display modes provide per-agent visibility: `display: { role:worker: [subject, body, { mask: "from" }], role:planner: [{ ref: "from" }, { ref: "message_id" }] }`. Different agent phases receive different field projections, and matching `role:*` llm labels select the corresponding display mode by default.
- Handles provide opaque cross-phase identity for record values. LLMs receive handles (e.g. `h_x9k2`) instead of copyable literals for masked and handle-only fields. The runtime resolves handles back to live values with proof labels at the tool-call boundary. Value-side `.mx.handle` / `.mx.handles` and grouped `@mx.handles` expose the active bridge-scoped handle view. Handles are root-scoped and survive across agent phases, enabling secure data flow from read-phase through planning to write-phase.
- `@fyi` namespace for agent awareness. `@fyi.known()` discovers proof-bearing handles for control args. `@fyi.shelf` provides display-projected access to shelf slot contents.
- `@fyi.known()` discovers proof-bearing handles in the registry. No-arg returns all handles; `@fyi.known("send_email")` filters by operation fact requirements. Implicitly available to LLMs with write tools and declared `controlArgs`.
- Value-scoped attestations: `known` and `known:*` labels are now scoped to exact values rather than conversation-wide. A tool result carrying `known` transfers its attestation only when the model passes the exact value unchanged — transformed or different values lose the attestation.
- Trusted data field subdivision: `data: { trusted: [title], untrusted: [body] }` separates safe-to-read fields (taint cleared, no fact labels) from injection-surface fields (taint preserved).
- Tool catalogs now accept `inputs: @record`, deriving surfaced params, control/source args, strict input validation, and same-source correlation from the record instead of separate `expose` / `controlArgs` wiring.
- Input records now support top-level `exact`, `update`, `allowlist`, `blocklist`, and `optional_benign` policy sections for surfaced tools. `@policy.build(...)`, `mlld validate`, `@toolDocs()`, MCP notes, and editor highlighting all read the same record-backed contract.
- Tool catalog entries now accept `description`, `instructions`, and `can_authorize`. Catalog labels are applied to the surfaced tool call, and catalog `can_authorize` defaults merge into `@policy.build(...)` / `@policy.validate(...)`.
- `@toolDocs()`, injected `<tool_notes>` / `<authorization_notes>`, MCP tool descriptions, `mlld validate`, and editor syntax/LSP support now understand record-backed tool catalogs.
...
- Boundary input canonicalization: handles are the primary proof path at all security boundaries. Tool dispatch and `with { policy }` compilation resolve handle wrappers and bare handle strings. The policy builder auto-upgrades `known` values to `resolved` when an exact matching handle exists in the registry (Postel's Goldilocks — scoped canonicalization in one well-bounded place). Session-local preview/literal matching has been removed in favor of handle-first identity.
- Taint checks scoped to declared `controlArgs`: `no-untrusted-destructive` and `no-untrusted-privileged` now check only control arg parameters, not all arguments. Data args can carry taint without blocking the call.
- Positive checks (`no-send-to-unknown`, `no-destroy-unknown`) now accept any `fact:*` proof on control args without requiring specific field-name pattern matches.
- Policy compiler strips data args from authorization bundles at compile time. Planners include all args in authorization intent; the runtime keeps only security-relevant parameters.
- Authorization denial diagnostics distinguish failure reasons: `unlisted` (tool never authorized), `compile_dropped` (entry dropped during compilation — ambiguity, proof loss), `args_mismatch` (args don't match constraint), `proofless_resolved_value` (resolved value lacks handle), `known_not_in_task` (known literal absent from task text), `known_contains_handle` (handle in known bucket).
- Compact tool security tables replace verbose per-tool annotation blocks in LLM prompts (~6x token reduction). Worker tables show control args and discovery targets; planner tables show control args.
- `@toolDocs()` generates prompt-ready tool documentation from tool metadata. `"planner"` audience mode includes control args and authorization bucket references. Non-argument form infers tools from scope.
- URL exfiltration defense: runtime maintains `@mx.urls.registry` from external inputs, and the managed `no-novel-urls` rule blocks influenced exe arguments that introduce URLs not present in execution input context unless `policy.urls.allowConstruction` permits the domain.
- Narrow eligible extraction inputs with `sourceArgs` metadata and the `no-unknown-extraction-sources` rule
...
- `@shelf.write/read/clear/remove` now provide the explicit shelf slot-ref API. `@shelve(@slotRef, @value)` remains write sugar, and `@shelve.read/clear/remove` remain compatibility aliases, so wrapper code can read, clear, and remove by slot ref without losing the old shorthand.
- Box shelf scope now supports aliased slot refs on both `read` and `write`. Aliased entries surface at `@fyi.shelf.<alias>` and remain slot-ref-capable, so agents can read projected contents directly and pass the same alias into `@shelf.write()`, `@shelf.read()`, `@shelf.clear()`, `@shelf.remove()`, or `@shelve()`.
- `authorizations.deny` list on policy prevents specific tools from ever being planner-authorized. Checked in both `@policy.build` and `with { policy }` compilation.
- `policy.authorizations.can_authorize` on base policy declares which `role:*` executables may authorize which tools. Authorization identity now comes from immutable exe role labels, not display overrides.
- `@policy.build` accepts bucketed intent (`resolved`/`known`/`allow`). `resolved` requires handle-backed values (bare literals rejected as `proofless_resolved_value`). `known` requires uninfluenced source. The entire bucketed intent must come from uninfluenced sources.
- `exe llm` calls now automatically inject `<tool_notes>` into the system message with compact security tables — per-tool controlArgs, `@fyi.known()` discovery calls, deny list, and bucketed intent shape for planners. Annotations are inferred from tool metadata and active policy. No manual prompt assembly needed.
- `exe llm` calls now also inject `<authorization_notes>` for tools the caller role may authorize but cannot call directly. Tool docs and authorization docs share the same display-shaped metadata path.
- Collection-key invocation with policy matching: `@writeTools["send_email"](@args) with { policy }` matches policy against the collection key, not the underlying exe name. Single-object args are spread to named params using tool metadata.
- `handle` field type for records: `facts: [channel: handle]` requires a resolvable handle — plain strings fail validation. Enforces handle-bearing worker returns via `=> record`.
- `correlateControlArgs: true` on exe declarations signals that multiple control args must come from the same source record. The runtime includes same-source guidance in `<tool_notes>` for these tools.
- `resume` guard action for LLM exes: continues the existing LLM conversation without re-executing tool calls. After-guards and pre-guards can say `resume "fix the format"` to append a correction message to the LLM session. The runtime captures conversation state via `config._mlld.resume` and the provider module handles session continuation (e.g., `--resume` for Claude CLI). Resume runs `=> record` coercion on the new response but skips pipeline replay. Precedence: `deny > resume > retry > allow`. Errors clearly when no resumable LLM session exists. Any `_mlld.resume.continue === true` LLM call clears `config.tools` regardless of how the continuation was triggered.
- `updateArgs` on exe declarations: declares which args are mutable fields on the target. The runtime rejects update calls with no non-null changed fields. `@policy.build` drops update tools authorized as bare `allow` when `updateArgs` is declared (issue: `no_update_fields`). `updateArgs` must be disjoint from `controlArgs`.
- `exactPayloadArgs` on exe declarations: declares which payload fields must appear explicitly in the user's task text. `@policy.build(@intent, @tools, { task: @query })` validates these against the task text for values in `resolved`, `known`, and flat intent (issue: `payload_not_in_task`). `exactPayloadArgs` must be a subset of non-control params.
...
- Strict `->` / `=->` tool returns now inherit taint from the return expression itself instead of the exe's ambient execution state.
...
- Function-tool MCP bridges now cancel active and queued `tools/call` requests when the client socket closes, including pending imported MCP calls.
- MCP proxy clients now keep backend MCP servers alive while connected, and in-flight MCP requests clear idle timers. This prevents long-running OpenCode agent sessions from losing tool results after idle thinking gaps between tool calls.
...
- `mlld validate` and `mlld analyze` now scope `policy.authorizations` schema diagnostics to actual `/policy` declarations and statically analyzable `@policy.build(...)` / `@policy.validate(...)` callsites, so ordinary data objects with an `authorizations` field remain plain data.
- `mlld validate` and `mlld analyze` no longer reject `box.shelf.write` aliases just because the slot ref comes through a variable binding. Let-bound refs selected via field access or dynamic shelf lookup now validate the same way direct slot refs do, while non-slot values are still rejected at runtime.
