...
/exe @get(url) = {curl -s "@url"}
/exe @post(url, data) = {curl -s -X POST -H "Content-Type: application/json" -d '@data' "@url"}
/exe @auth_get(url, token) = {curl -s -H "Authorization: Bearer @token" "@url"}
/exe @auth_post(url, token, data) = {curl -s -X POST -H "Authorization: Bearer @token" -H "Content-Type: application/json" -d '@data' "@url"}
