...
describe('Unified variable syntax', () => {
  it('should parse text variables with {{variable}} syntax', async () => {
    ...
    // TextVar node
    expect(result.ast[1].type).toBe('TextVar');
    expect(result.ast[1].identifier).toBe('name');
    expect(result.ast[1].varType).toBe('text');
  ...
  it('should distinguish between text and data variables using fields', async () => {
    ...
    const textVar = result.ast.find(node => node.type === 'TextVar');
    expect(textVar).toBeDefined();
    expect(textVar?.varType).toBe('text');
    
    const dataVar = result.ast.find(node => node.type === 'DataVar');
    expect(dataVar).toBeDefined();
    expect(dataVar?.varType).toBe('data');
