...
describe('Parser', () => {
  ...
  describe('Variables', () => {
    it('should parse a text variable', async () => {
      ...
      expect(ast).toHaveLength(1);
      const node = ast[0] as TextVarNode;
      expect(node.type).toBe('TextVar');
    ...
    it('should parse a data variable', async () => {
      ...
      const { ast } = await parse(input);
      expect(ast).toHaveLength(2);
      const node = ast[1] as any;
      expect(node.type).toBe('DataVar');
    ...
    it('should parse a path variable', async () => {
      ...
      expect(ast).toHaveLength(1);
      const node = ast[0] as PathVarNode;
      expect(node.type).toBe('PathVar');
      expect(node.identifier).toBe('HOMEPATH');
      expect(node.isSpecial).toBe(true);
    });
  });
