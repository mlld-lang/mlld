...
describe('Phase 4B: Implementation Plan for Variable-based Add Transformations', () => {
  /**
   * This test file doesn't execute actual tests, but provides the implementation plan
   * for fixing the variable-based add transformation issue in Phase 4B.
   */
  it('should document the implementation plan for Phase 4B fix', () => {
    /*
      PHASE 4B: FIX FOR VARIABLE-BASED ADD TRANSFORMATION
      
      ISSUE:
      When an @add directive uses a variable reference with field access (e.g., {{role.architect}}),
      ...
      add directives specially when in transformation mode. It needs to resolve the variable 
      and its fields directly rather than relying on the generic transformation lookup.
      
      IMPLEMENTATION PLAN:
      
      1. Modify OutputService.nodeToMarkdown method to handle variable-based add directives
         ...
         a. In the case where node.type === 'Directive':
            - Add special handling for add directives with variable references
      ...
      ```typescript
      // In OutputService.nodeToMarkdown, add this special case for add directives:
      if (node.type === 'Directive' && node.directive.kind === 'add') {
        // Special handling for variable-based add directives in transformation mode
        if (node.directive.path && 
          ...
          logger.warn(`Could not resolve variable reference ${varName} in add directive`);
        }
      ...
      a. Update add-transformation-e2e.test.ts to properly test variable-based add directives:
         - Remove the temporary workaround
         - Verify that `@add {{role.architect}}` is properly transformed to "Senior architect"
         
      b. Update add-transformation-variable-fix.test.ts:
         - Remove the temporary workaround
      ...
      c. Add a new test case specifically for field access in variable embeds:
         ...
         - Use an add directive with field access (e.g., `@add {{user.info.name}}`)
  ...
  it('should document code snippets for implementation', () => {
    ...
    /**
     * Example implementation of a variable resolver for add directives
    ...
    private async nodeToMarkdown(node: MeldNode, state: IStateService): Promise<string> {
      ...
      switch (node.type) {
        ...
        case 'Directive':
          ...
          // Handle add directives
          if (kind === 'add') {
            // NEW PHASE 4B CODE: Special handling for variable-based add directives in transformation mode
            if (directive.directive.path && 
                typeof directive.directive.path === 'object' && 
    ...
    */
    ...
    /**
     * Example of a complete test case for variable-based add transformation
     */
    /*
    it('should properly transform variable-based add directives with field access', async () => {
      // Create file with data variable and add directive
      const testContent = '@data user = { "info": { "name": "John Doe" } }\n@add {{user.info.name}}';
      await context.services.filesystem.writeFile('variable-add.meld', testContent);
      
      // Process with transformation mode enabled
      const result = await main('variable-add.meld', {
