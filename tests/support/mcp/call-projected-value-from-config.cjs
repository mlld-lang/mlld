...
const [, , configPath, lookupTool, lookupArgsJson = '{}', valuePath, targetTool, targetArgsJson = '{}', targetArgName = 'recipient'] = process.argv;

if (!configPath || !lookupTool || !valuePath || !targetTool) {
  fail(
    'Usage: node call-projected-value-from-config.cjs <configPath> <lookupTool> <lookupArgsJson> <valuePath> <targetTool> <targetArgsJson> [targetArgName]'
  );
}
...
(async () => {
  ...
  const projectedValue = getPathValue(parsedLookup, valuePath);
  if (projectedValue === undefined) {
    fail(`Expected value at path '${valuePath}'`);
  }
