...
`policy.authorizations` compiles to internal privileged guards that enforce a default-deny envelope over `tool:w` operations. These guards can override managed label-flow denials for matching calls, enabling task-scoped authorization where a planner pins specific values and the runtime enforces them. See `policy-authorizations` for details.
