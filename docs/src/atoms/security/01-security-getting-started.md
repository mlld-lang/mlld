...
Declare control args on the write executable itself with `with { controlArgs: [...] }`. Tool collections can restate or tighten that metadata for a specific exposure. If the planner includes data args in the authorization, the runtime strips them — only control args matter for constraints. Invalid authorization fragments fail closed during `with { policy }` activation, so no partial authorization envelope is installed.
