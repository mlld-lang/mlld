...
Declare surfaced write-tool control args with `inputs: @record` and put every security-relevant field in `facts:`. Base policy uses `authorizations.can_authorize` to declare which roles can authorize which write tools. The planner then emits bucketed authorization intent for `@policy.build`. If the planner includes data args in that intent, the runtime strips them — only surfaced fact fields matter for constraints. Invalid authorization intent fails closed during compilation, so no partial authorization envelope is installed.
