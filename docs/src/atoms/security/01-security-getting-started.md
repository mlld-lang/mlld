...
Declare control args on the write executable itself with `with { controlArgs: [...] }`. Tool collections can restate or tighten that metadata for a specific exposure. Base policy uses `authorizations.authorizable` to declare which roles can authorize which write tools. The planner then emits bucketed authorization intent for `@policy.build`. If the planner includes data args in that intent, the runtime strips them — only control args matter for constraints. Invalid authorization intent fails closed during compilation, so no partial authorization envelope is installed.
