...
This makes documentation executable.
...
Each iteration: fresh context, load state from disk, do work, write state back. No accumulated garbage from previous loops. The `@state` variable is SDK-controlled, so you can start and stop Ralph loops programmatically from Node, Python, Go, or Rust. See [SDK](/docs/sdk/) for SDK integration.
