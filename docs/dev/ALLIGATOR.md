...
`<file>` loads content. Everything is StructuredValue with `.text` (display), `.data` (parsed), `.mx` (metadata). Auto-unwraps to content in mlld contexts. Use `.keep` when passing to JS/Node.
...
- `.mx`: Metadata (filename, relative, absolute, url, fm, tokens, tokest, etc.)
...
/show @readme                    # Content
/show @readme.mx.filename       # "README.md"

/var @docs = <docs/*.md>
/show @docs                      # Concatenated with \n\n
/show @docs.mx.fileCount        # 15
...
/var @docs = <docs/*.md>
/show @docs                     # Concatenated with \n\n
/show @docs.mx.fileCount       # Number of files
```
...
Metadata is accessible in mlld contexts via `.mx`:

```mlld
/var @readme = <README.md>
/show @readme.mx.filename       # "README.md"
/show @readme.mx.relative       # "README.md"
/show @readme.mx.tokens         # 1523
/show @readme.mx.tokest         # ~1500

/var @doc = <doc.md>
/show @doc.mx.fm.title          # Frontmatter title
/show @doc.mx.fm.author         # Frontmatter author

/var @url = <https://example.com>
/show @url.mx.url               # "https://example.com"
/show @url.mx.domain            # "example.com"
/show @url.mx.title             # Page title
```

Token estimates come from shared helper (`core/utils/token-metrics.ts`). Same heuristics apply whether string originates from `<file>` loads, `/var` assignments, or downstream evaluations. Every variable exposes metrics via `.mx`.
...
/exe @processFile(@config.keep) = js {
  return JSON.stringify({
    filename: config.mx.filename,
    tokens: config.mx.tokens,
...
/exe @validate(@file.keep) = js {
  return `${file.mx.filename} has ${file.mx.tokens} tokens`;
}
```
...
- **Field access**: `@file.mx.filename` accesses metadata. `@file` in templates uses `.text`.
